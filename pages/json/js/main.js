$(document).ready(function(){
    ...
    function createMap(){
        $.getJSON('json/countries.json', function(country) {
            ...
            $('.country-block').append('<h3 class="current-country"></h3>').append(locationBlock).append(blockCurrent).append('<div class="block-location">{{location.name}}</div>');
            $('.active-location').css({"display": "none"});
            let countryCodeValue = 'json/'+countryCode.toLocaleLowerCase()+'.json';
    ...
    function positionLocation(){
        ...
        if ($(".land.active").hasClass('right')) {
             widthPath = $(".land.active")[0].getBoundingClientRect().width / 3 * 2;
        }
        ...
        let topSum = topPosition - positionBlockTop + heightPath - 20;
